...
namespace Browl.Service.MarketDataCollector.API.Controllers
{
	public class CategoriesController : MainController
	{
		...
		public async Task<IActionResult> PostAsync([FromBody] CategorySaveResource resource)
		{
			...
			Domain.Communication.Response<Category> result = await _categoryService.SaveAsync(category);
			...
			CategoryResource categoryResource = _mapper.Map<CategoryResource>(result.Resource!);
		...
		{
			Category category = _mapper.Map<Category>(resource);
			Domain.Communication.Response<Category> result = await _categoryService.UpdateAsync(id, category);
		...
		{
			Domain.Communication.Response<Category> result = await _categoryService.DeleteAsync(id);
