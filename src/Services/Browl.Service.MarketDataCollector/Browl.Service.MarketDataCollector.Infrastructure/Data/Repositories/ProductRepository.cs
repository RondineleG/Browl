...
public class ProductRepository : BaseRepository, IProductRepository
{
	...
	{
		var queryable = _browlDbContext.Products
		...
		var products = await queryable.Skip((query.Page - 1) * query.ItemsPerPage)
	...
	public async Task<Product> FindByIdAsync(int id) => await _browlDbContext.Products
							 .Include(p => p.Category)
							 .FirstOrDefaultAsync(p => p.Id == id);

	public async Task AddAsync(Product product) => _ = await _browlDbContext.Products.AddAsync(product);

	public void Update(Product product) => _ = _browlDbContext.Products.Update(product);

	public void Remove(Product product) => _ = _browlDbContext.Products.Remove(product);
}